...
class Bottleneck(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, stride, C, d):
        ...
        self.fc_spatial1 = nn.Conv2d(conv3_out, conv3_out, kernel_size=1, groups=conv3_out)
        self.fc_spatial2 = nn.Conv2d(conv3_out, conv3_out, kernel_size=2, groups=conv3_out)
        self.fc_spatial4 = nn.Conv2d(conv3_out, conv3_out, kernel_size=4, groups=conv3_out)
        self.fc_bn = nn.BatchNorm2d(conv3_out)
    ...
    def forward(self, x):
        ...
        se_out1 = self.pool1(res)
        se_out2 = self.pool2(res)
        se_out4 = self.pool4(res)
        se_out1 = self.fc_spatial1(se_out1)
        se_out2 = self.fc_spatial2(se_out2)
        se_out4 = self.fc_spatial4(se_out4)
        se_out = se_out1 + se_out2 + se_out4
        se_out = F.relu(self.fc_bn(se_out), inplace=True)
...
class SPSE_ResNeXt(nn.Module):
    def __init__(self, block, C, d, layers, num_classes = 1000):
        self.inplanes = 64
        super(SPSE_ResNeXt, self).__init__()
...
def resnext50_spse():
    return SPSE_ResNeXt(block=Bottleneck,C=32,d=4,layers=[3,4,6,3])

def resnext101_spse():
    return SPSE_ResNeXt(block=Bottleneck,C=32,d=4,layers=[3,4,23,3])